    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title"><i class="fa fa-plus"></i> {$_L['Add New Lead']}</h3>
            </div>
            <div class="panel-body">
                <form class="form-horizontal" method="post" action="{$_url}plugin/leads&action=add">
                        ...
                        <div class="col-md-6">
                            <select class="form-control" name="status">
                                {foreach explode(',', $_c['lead_statuses']) as $status}
                                    <option value="{$status}" {if $status eq 'New'}selected{/if}>{$status}</option>
                                {/foreach}
                            </select>
                        </div>
                    </div>
